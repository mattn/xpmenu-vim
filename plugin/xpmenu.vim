...
function! LoadLibrary(dllname)
  let hModule = '0'
  silent let hModule = libcall(s:libfile, 'dllload', a:dllname)
  return hModule
endfunction
...
function! FreeLibrary(hModule)
  let dwRet = 0
  silent let dwRet = libcall(s:libfile, 'dllfree', a:hModule)
  return dwRet
endfunction
...
function! CallLibrary(hModule, pszFuncName, pszArgument)
  let pszRet = ''
  silent let pszRet = libcall(s:libfile, 'callfunc', a:hModule.','.a:pszFuncName.','.a:pszArgument)
  return pszRet
endfunction
